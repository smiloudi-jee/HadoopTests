package com.natixis.minicluster.core;

import com.natixis.minicluster.modules.hbase.HBaseServiceHandle;
import com.natixis.minicluster.modules.hive.HiveBootstrap;
import com.natixis.minicluster.modules.hive.HiveServiceHandle;
import com.natixis.minicluster.modules.mapreduce.MapReduceServiceHandle;
import com.natixis.minicluster.modules.pig.PigServiceHandle;
import com.natixis.minicluster.modules.rest.RestBootstrap;
import com.natixis.minicluster.modules.rest.RestServiceHandle;
import com.natixis.minicluster.modules.spark.SparkBootstrap;
import com.natixis.minicluster.modules.spark.SparkServiceHandle;
import com.natixis.minicluster.modules.yarn.YarnServiceHandle;
import com.natixis.minicluster.util.PrinterTools;
import com.natixis.minicluster.util.StarterUtils;
...
public final class ClusterRuntime implements AutoCloseable {
    ...
    public static ClusterRuntime start(ClusterRuntimeConfig cfg) throws Exception {
        ...
        // 8. YARN
        Optional<YarnServiceHandle> yarnHandle = Optional.empty();
        if (cfg.isYarnEnabled()) {
            yarnHandle = Optional.of(
                    com.natixis.minicluster.modules.yarn.YarnBootstrap.startYarn(hadoopConf)
            );
        }
        ...
        if (cfg.isMapReduceEnabled()) {
            mapReduceHandle = Optional.of(
                    com.natixis.minicluster.modules.mapreduce.MapReduceBootstrap.startLocalMapReduce(hadoopConf, hdfsUri)
            );
        }

        // 10. HBase
        Optional<HBaseServiceHandle> hbaseHandle = Optional.empty();
        if (cfg.isHBaseEnabled()) {
            assertNotNull(com.natixis.minicluster.modules.hbase.HBaseBootstrap.startHBase(),
                    "HB aseHandle should not be null");
            hbaseHandle = Optional.of(
                    com.natixis.minicluster.modules.hbase.HBaseBootstrap.startHBase()
            );
        }
        ...
        if (cfg.isPigEnabled()) {
            pigHandle = Optional.of(
                    com.natixis.minicluster.modules.pig.PigBootstrap.startPig(hadoopConf, hdfsUri)
            );
        }
